...
export default function AgentsPanel() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8">
      <div>
        <h1 className="text-lg font-semibold text-zinc-100">Agents &amp; tasks</h1>
        <p className="mt-1 text-sm text-zinc-500">
          Legacy agent-platform MVP. Store provisioning lives on Stores.
        </p>
      </div>

      <section>
        <h2 className="mb-4 text-base font-semibold text-zinc-200">Agents</h2>
        <div className="mb-4 flex gap-3">
          <input
            value={newAgentName}
            onChange={(e) => setNewAgentName(e.target.value)}
            placeholder="Agent name"
            className="flex-1 rounded-lg border border-zinc-700 bg-zinc-900 px-4 py-2 text-sm placeholder-zinc-500 focus:border-emerald-500 focus:outline-none"
          />
          <input
            value={newAgentDesc}
                ...
                <div className="mt-3 flex items-center gap-2 text-xs text-zinc-400">
                  ...
                  <span
                    className={`rounded px-2 py-0.5 ${
                      agent.status === "idle" ? "bg-zinc-800" : "bg-emerald-900 text-emerald-300"
                    }`}
                  >
                    {agent.status}
                  </span>
      ...
      <section>
        <h2 className="mb-4 text-base font-semibold text-zinc-200">Tasks</h2>
        <div className="mb-4 flex gap-3">
          <input
            value={newTaskTitle}
